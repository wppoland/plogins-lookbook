...
<div class="lookbook-editor" data-lookbook-editor>
    <p class="lookbook-editor__intro">
        <?php esc_html_e('Set the lookbook image using the Featured image box, then add a hotspot for each product. Position each one with X and Y as a percentage from the top-left of the image (0-100), and enter the product ID to link.', 'bildaro'); ?>
    </p>
    ...
    <div class="lookbook-editor__rows-wrap">
        <table class="widefat lookbook-editor__table" data-lookbook-rows>
                <thead>
                    <tr>
                        <th scope="col" class="lookbook-editor__col-num"><?php esc_html_e('#', 'bildaro'); ?></th>
                        <th scope="col"><?php esc_html_e('X %', 'bildaro'); ?></th>
                        <th scope="col"><?php esc_html_e('Y %', 'bildaro'); ?></th>
                        <th scope="col"><?php esc_html_e('Product ID', 'bildaro'); ?></th>
                        <?php do_action('lookbook/admin_hotspot_row_header'); ?>
                        <th scope="col" class="lookbook-editor__col-actions"><span class="screen-reader-text"><?php esc_html_e('Actions', 'bildaro'); ?></span></th>
                    </tr>
                </thead>
                <tbody data-lookbook-rows-body>
                    ...
                    foreach ($rows as $i => $row) :
                        ...
                        <tr class="lookbook-editor__row" data-lookbook-row>
                            ...
                            <td>
                                <label class="screen-reader-text" for="lookbook_x_<?php echo esc_attr((string) $i); ?>"><?php esc_html_e('X percentage', 'bildaro'); ?></label>
                                <input
                                    type="number"
                            ...
                            <td>
                                <label class="screen-reader-text" for="lookbook_y_<?php echo esc_attr((string) $i); ?>"><?php esc_html_e('Y percentage', 'bildaro'); ?></label>
                                <input
                                    type="number"
                            ...
                            <td>
                                <label class="screen-reader-text" for="lookbook_pid_<?php echo esc_attr((string) $i); ?>"><?php esc_html_e('Product ID', 'bildaro'); ?></label>
                                <input
                                    type="number"
            ...
            <p>
                <button type="button" class="button lookbook-editor__add" data-lookbook-add>
                    <?php esc_html_e('+ Add hotspot', 'bildaro'); ?>
                </button>
        </p>
    </div>
</div>
